...
Ao chamar `sync` ou `get` você está livre para combinar localizadores de correspondência em qualquer ordem que você achar necessário para 
encontrar qualquer evento Time State criado por `cue` ou `set`. Vamos ver um exemplo doido:
