...
1. "/path/to/samples/foo.wav" - en tekststreng som representerer en gyldig sti til et sample
2. "/path/to/samples/foo.wav" - en tekststreng som representerer en gyldig sti til et sample

The `sample` fn will first gather all sources and use them to create a large list of candidates. This list is constructed by first adding all valid paths and then by adding all the valid `.flac`, `.aif`, `.aiff`, `.wav`, `.wave`, `.mp3` files contained within the directories.
