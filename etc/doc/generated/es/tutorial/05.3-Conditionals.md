...
Típico de lenguajes de programación, la noción de 'sí' está representada por el término `true` y la noción de 'no', por `false`. Así que debemos encontrar una pregunta que nos dé una respuesta `true` o`false` que es exactamente lo que hace `one_in` .
