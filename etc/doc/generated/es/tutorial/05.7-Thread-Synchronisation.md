...
Eres libre de utilizar cualquier nombre que quieras a los mensajes de `cue` - no sólo `:tick`. Sólo debes asegurarte que los otros hilos estén `sync` sincronizándose en el nombre correcto - de otro modo se quedarán esperando por siempre (o hasta presionar el botón de Parar).
