...
Como hemos visto, por defecto, el FX `:sound_out` da salida a una mezcla mono de la entrada estéreo a un canal específico, además de pasar la alimentación estéreo al contexto exterior (como es de esperar). Si la salida de una mezcla mono no es precisamente lo que quieres hacer, hay varias opciones alternativas. En primer lugar, utilizando la opción `mode:` puede elegir la salida de sólo la señal de entrada izquierda o derecha a la tarjeta de audio. O puede utilizar la función `:sound_out_stereo` para enviar la señal a dos salidas consecutivas de la tarjeta de sonido. Consulte la documentación de la función para obtener más información y ejemplos.
