...
(defsynth sonic-pi-tb303_gated
   ...
   (let [decay_level        (select:kr (= -1 decay_level) [decay_level sustain_level])
        ;;  level              (/ sustain_level attack_level) ;; hack, while env-adsr has to be used
         level              1
         ...
         freq               (midicps note)
        ;;  env                (env-gen (env-adsr-ng attack decay sustain release attack_level decay_level env_curve) :action FREE)
        ;;  env                (env-gen (env-adsr-ng attack decay sustain release attack_level decay_level sustain_level env_curve) :action FREE)
        ;;  env                (env-gen (env-adsr attack decay 0.1 release 5 env_curve) gate :action FREE) ;; this worked, but it is not a full adsr curve with 4 levels and 3 time intervals
        ...
        ;;  env                (env-gen (core/gated-shaped-adsr 2 1 4 4 6 0.1 1 1 3) gate :action FREE) ;; works, just keep the key pressed when testing; test with real params
  ...
  (defsynth sonic-pi-hoover_gated
 ...
 (defsynth sonic-pi-zawa_gated [note 52
                          note_slide 0
                          note_slide_shape 1
                          note_slide_curve 0
                          amp 1
 ...
 (defsynth sonic-pi-prophet_gated
   ...
   (core/save-synthdef sonic-pi-tb303_gated)
   (core/save-synthdef sonic-pi-supersaw_gated)
   (core/save-synthdef sonic-pi-hoover_gated)
   (core/save-synthdef sonic-pi-prophet_gated)
   (core/save-synthdef sonic-pi-zawa_gated)
   )
